...
if data_type == "Dataverse":
    ...
    if st.button("생성", key='submit_dataverse'):
        res = api_call.create_dataverse(parent,name,alias,st.session_state.email_contacts,affiliation,description,dataverse_type)
        st.write("응답 내용:", res)
    ...
    uploaded_file = st.file_uploader("PDF 파일을 업로드 하세요", type=["pdf"],accept_multiple_files=True)
    if st.button("생성", key='submit_dataset'):
        res = api_call.create_dataset(parent,title,name,university,st.session_state.email_contacts,subject)
        st.write("응답 내용:", res)
    elif st.button("삭제", key='delete_dataverse'):
        res = api_call.delete_dataverse(title)
        st.write("응답 내용:", res)
    elif st.button("검색", key='search_dataset'):
        res = api_call.search_dataset(title)
        st.write("응답 내용:", res)
elif data_type == "File":
    st.header("File upload")
    parent = st.text_input("상위 dataset PID")
    uploaded_files = st.file_uploader("파일을 업로드 하세요", type=["txt", "csv", "pdf", "xlsx"], accept_multiple_files=True)
    if st.button("파일 업로드"):
